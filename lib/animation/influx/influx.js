(function($){
    var InfluxAnimation = {
        ...
        init: function() {
            ...
            roller.axisMove = module.setAxis();
        ...
        touchEndHandler: function(data) {
            ...
            if(commonTouchDir === lastTouchDir) {
                var influxDir = roller.options.influxDir,
                    invertDir = { next: 'prev', prev: 'next' };

                if( influxDir === 'from-left' ||
                    influxDir === 'from-top'
                ) {
        ...
        setDirection: function(index) {
            var module  = this,
                ...
                direction = roller.options.influxDir;
        ...
        influxAnimateTransform: function(index, speed) {
            ...
            el.trigger('beforeMove', { index: index });
        ...
        setAxis: function() {
            var animateDir = this.roller.options.influxDir;

            if(animateDir === 'from-right' || animateDir === 'from-left') {
                return 'x';
            }

            if(animateDir === 'from-bottom' || animateDir === 'from-top') {
                return 'y';
            }
        }
    };
